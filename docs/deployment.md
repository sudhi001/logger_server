...
|---|---|---|---|---|
| `sudhis/logger_server:3.0.0` | `scratch` (static musl) | 2.5 MB compressed | 11.1 MB | Production. The default |
...
services:
  logger:
    image: sudhis/logger_server:3.0.0
...
| Setting | Value |
|---|---|
| Image URL | `docker.io/sudhis/logger_server:3.0.0` |
...
```sh
docker pull sudhis/logger_server:3.0.0
docker stop logger && docker rm logger
docker run -d --name logger ... sudhis/logger_server:3.0.0
```
